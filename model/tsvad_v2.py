...
class Model(nn.Module):
    def __init__(self, out_channels=[64, 64, 128, 128], rproj=128, nproj=160, cell=896):
        ...
        self.rnn_speaker_detection1 = BLSTMP(3*rproj, cell, num_layers=2)
        self.rnn_speaker_detection2 = BLSTMP(3*rproj, cell, num_layers=2)
        self.rnn_speaker_detection3 = BLSTMP(3*rproj, cell, num_layers=2)
        self.rnn_speaker_detection4 = BLSTMP(3*rproj, cell, num_layers=2)
        self.rnn_combine = BLSTMP(8*nproj, cell)

        self.output_layer1 = nn.Linear(2*nproj, 1)
        self.output_layer2 = nn.Linear(2*nproj, 1)
        self.output_layer3 = nn.Linear(2*nproj, 1)
        self.output_layer4 = nn.Linear(2*nproj, 1)
    ...
    def forward(self, batch):
        ...
        sd_in  = self.linear(sd_in)                              # B x 4 x T x 384
        sd_out1 = self.rnn_speaker_detection1(sd_in[:, 0])       # B x T x 320 
        sd_out2 = self.rnn_speaker_detection2(sd_in[:, 1])
        sd_out3 = self.rnn_speaker_detection3(sd_in[:, 2])
        sd_out4 = self.rnn_speaker_detection4(sd_in[:, 3])

        sd_out = torch.cat((sd_out1, sd_out2, sd_out3, sd_out4), dim=-1) # B x T x 1280
        
        outputs = self.rnn_combine(sd_out)                       # B x T x 320
        pred1   = self.output_layer1(outputs)                     # B x T x 1
        pred2   = self.output_layer2(outputs)
        pred3   = self.output_layer3(outputs)
        pred4   = self.output_layer4(outputs)
        preds   = torch.cat((pred1, pred2, pred3, pred4), dim=-1)# B x T x 4
